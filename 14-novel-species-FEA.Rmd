...
The predicted proteome was also annotated with [STRING](https://string-db.org/) v 12.0. As of version 12, STRING now includes a `Add any organism to STRING / Annotate proteome` feature ([Szklarczyk etl al 2023](https://pmc.ncbi.nlm.nih.gov/articles/PMC9825434/)). The axolotl proteome was uploaded to STRING and annotation performed on STRING servers. The [resulting annotation](https://version-12-0.string-db.org/organism/STRG0A90SNX) is persistent and shareable and can be used for all of STRING's search functions including ORA (`Multiple proteins`) and GSEA (`Proteins with Values/Ranks`)
...
2. Reformat these to create the axolotl database files required by `clusterProfiler` for GO and KEGG FEA analysis
...
5. Reformat the files imported at (1) to create the axolotl database files required by `WebGestaltR` for GO and KEGG FEA analysis
6. Run ORA and GSEA with `WebGestaltR` package (function name is also `WebGestaltR`) 
7. Visualise `WebGestaltR` results in the saved output HTML report
8. Run ORA with `STRING` online using custom axolotl annotation
9. Compare `STRING` ORA web results to those generated using the `emapper` annotations in R 

## R-based FEA

### clusterProfiler

This tool can perform ORA or GSEA for any organism with the provision of custom `TERM2GENE` and `TERM2NAME` files.  `TERM2GENE` maps the species gene ID to database (eg GO, KEGG) terms, and `TERM2NAME` maps the terms to their descriptive names. 
...
![t2name](images/axolotl-term2name.png)

There are then provided to the universal enrichment functions `GSEA` and `enricher` (ORA). It is essential that the gene lists provided have the same gene IDs as those in the `TERM2GENE` file. 
...
**Example .gmt format:**  <!-- caption text in italics -->
...
These files are then provided to the single FEA function within the package. The function has the same name as the package, `WebGestaltR`, with the parameter `enrichMethod` controlling whether ORA, GSEA or NTA is performed. 

The `.gmt` file is provided to the parameter `enrichDatabaseFile` and the `.des` file is provided to the parameter `enrichDatabaseDescriptionFile`. Specifying `organism = "others"` is also required to run the FEA analysis with the custom databse files. Same as with `clsuterProfiler`, it is essential that the `.gmt` file has the same gene IDs as those in query gene list. 

![webgr-others](images/webgestaltr-others-organisms.png)
...
`eggNOG emapper` appears more stringent than STRING in its similarity matches between the axolotl putative proteins and annotated orthologs. The table below compares the number of annotations yielded from the 47,196 putative axolotl proteins from both tools. 
...
This suggests that the `emapper` results may be more specific, while the `STRING` results may be more sensitive. Using both methods for your novel speciues analysis may help validate and focus your FEA findings.    