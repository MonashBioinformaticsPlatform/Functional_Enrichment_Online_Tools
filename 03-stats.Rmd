...
> _Note:_ This is just a toy calculator for this training, it is quite limited. You can also use [online tools](https://www.socscistatistics.com/tests/fisher/default2.aspx) to play with.
...
Hypergeometric Test: This test calculates the probability of observing the given number of genes from a specific category (e.g., a pathway) in the gene list (differentially expressed genes) by chance. It models the situation where you draw a sample (the gene list) from a finite population (the background of all genes), and success is defined as a gene being in the category (e.g., belonging to the pathway).
...
In R, this can be can be achieved by:
```{r, eval=FALSE}
