...
The R package `multienrichjam` has a function `enrichDF2enrichResult` that converts dataframe type results from other FEA tools to the format required for `enrichplot`!  
 ...
 `multienrichjam` has a lot of dependencies and has not been installed on these VMs so we will not be exploring today. However, this functionality and flexibility is pretty cool, so if you wanted to install this on your own computer outside the workshop, below code for installing :-)  
