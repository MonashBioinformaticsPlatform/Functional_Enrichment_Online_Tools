...
`clusterProfiler` has a number of `gse<DB>` functions specifically designed to perform GSEA on key databases without having to manually import the database:
...
There are also database-specific ORA functions `enich<DB>` however we won't be running those today since we have covered ORA with `gprofiler`. 


We will run GSEA over the GO database. If we choose `ALL` for the `ont` (ontology) parameter,  we can run over MF, BP and CC in one run. 
...
Next we will visualise the GSEA results with `enrichplot`. 
