...
publish_gosttable( ora_multi, highlight_terms = NULL, use_colors = TRUE, show_columns = c("source", "term_name", "term_size", "intersection_size"), filename, ggplot = TRUE )
```


```{r view multiquery rsult table}
head(ora_multi$result)
```

```{r}
# Save ora_multi$result as a TSV file
#write.table(ora_multi$result, "gprofiler_multiquery_results.tsv", sep = "\t", row.names = FALSE, quote = FALSE)


ora_multi_reordered <- ora_multi$result[, c("source", "term_name", "term_id", "p_value", "term_size", "query_size", "intersection_size", "effective_domain_size")]

head(ora_multi_reordered)

write.csv(ora_multi_reordered, "gprofiler_ORA_results.csv", row.names = FALSE)
