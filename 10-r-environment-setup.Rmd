...
You have previosuly been provided with an IP address, user name and password for a VM for this workshop. Th VM runs RStudio and has all of the required R libraries for today's workshop pre-installed. 
...
<img src="images/vm-rstudio-open.png" style="border: none; box-shadow: none; background: none;">
...
- **TBA:** - inset VM screenshots and instructions
...
- **TBA** - add link to download input data and R notebooks for day2. 
...
The workshop data you have downloadaed contains 4 R notebooks required to run the analyses. After each activity, you may choose to knit the notebook to HTML to have a static record of your work :open_book: 
...
Saving the workspace image saves all objects from the session such as your variables and dataframes. This can save time if you need to close an analysis part way through and continue later. However, this can have drawbacks such as the clashes as mentioned, as well as large file size, relic objects cluttering the workspace, old objects conflicting with new ones, etc. Since we will be performing discrete analysis tasks today, and not continuing on a growing body of work, selecting `Dont save` will be the most appropriate. 
...
&#x27A4; On the `Files` pane, open the `day2_inputData` folder and confirm that it contains the input data file `Pezzini_DE.txt`

This will be our input file for the first activity. 

<p>&nbsp;</p>  <!-- insert blank line -->

&#x27A4; Load the notebook `day2_Rnotebooks/gprofiler2.Rmd` by selecting `File` &rarr; `Open file`, or use the keyboard shortcut `ctrl + o`. 

The code chunk labelled `working directory` defines the working directory. Setting the working directory allows us to load input data and save output data with filepaths *relative* to this directory. 
...
- **TBA**: what path is it that we want them to be in? 

<p>&nbsp;</p>  <!-- insert blank line -->

Note that setting the working directory for the notebook and for the console are 2 different things! 

&#x27A4; Copy paste the `setwd()` command to the console and run that command in order to update your console working directory to match the notebook. 

<p>&nbsp;</p>  <!-- insert blank line -->

- **TBA** - confirm that it will be this path or else update 
